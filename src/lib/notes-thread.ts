...
export function parseNoteThread(content: string): NoteTurn[] {
  ...
  const push = (from: number, to: number, boundary?: Boundary) => {
    ...
    // A "client reply" / "client sign-off" / "client message" label is only
    // ever written by src/lib/portal/reply.ts — the one audited client write
    // path — so it's a reliable signal that this turn came from a portal
    // client rather than the operator or an agent.
    const role: TurnRole = /client (reply|sign-off|message)/i.test(meta.label ?? "") ? "client" : roleFor(meta.author);
